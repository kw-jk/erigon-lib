...
var (
	...
	ErrAccountStateNotFound = errors.New("state for account key not found")
	ErrStorageStateNotFound = errors.New("state for storage key not found")
)
...
func (cell *Cell) setAccountPlainKey(plainKey []byte) {
	cell.apl = len(plainKey)
	copy(cell.apk[:], plainKey)
}

func (cell *Cell) setCode(codeHash []byte) { copy(cell.CodeHash[:], codeHash) }
...
func (hph *HexPatriciaHashed) computeCellHash(cell *Cell, depth int, buf []byte) ([]byte, error) {
	...
	if cell.spl > 0 {
		...
		if singleton {
			...
			aux := make([]byte, 0, 33)
			if aux, err = hph.leafHashWithKeyVal(aux, cell.downHashedKey[:64-hashedKeyOffset+1], rlp.RlpSerializableBytes(cell.Storage[:cell.StorageLen]), true); err != nil {
				return nil, err
			}
...
func (hph *HexPatriciaHashed) fold() (BranchData, []byte, error) {
	...
	for i, hashedKey := range hashedKeys {
		...
		if len(plainKey) == hph.accountKeyLen {
			...
			if stagedCell.isEmpty() {
				...
				cell.setAccountPlainKey(plainKey)
				cell.Balance.SetBytes(stagedCell.Balance.Bytes())
				cell.Nonce = stagedCell.Nonce
				cell.setCode(stagedCell.CodeHash[:])
